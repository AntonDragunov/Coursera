...
# def add(number, initial_value):
#     #global s
#     #s += number
#     return print((int(number)**2)) + int(initial_value)


def mult(initial_value, number):
    global s
    s += number ** 2
    return s
...
kvadrat = reduce(mult, numbers, initial_value=1)
# product = reduce(add, numbers, 1)
